...
int handleInstr(unsigned short instr, unsigned short arg1, unsigned short arg2, FILE *ram, FILE *disk) {
    ...
    case 0x15:
        writeWord(rv1, readWord(sp, ram), ram);
        writeWord(readWord(sp, ram)-2, sp, ram);
        break;
    
    case 0x16:
        writeWord(readWord(sp, ram)+2, sp, ram);
        writeWord(readWord(readWord(sp, ram), ram), rv1, ram);
        break;
    ...
    */
    ...
    for (int i = 0; $ < RAMSIZE;i++) {
        ...
        if (instr == 0x00) {
            ...
            instrSize = 2;
        } else if ((instr%16) == 0x04) {
            instrSize = 2;
        } else if ((instr%16) == 0x05) {
            instrSize = 2;
        } else {
            instrSize = 0;
        }
        ...
        if (readWord(SENDCHAR, ram) == 0xffff) {
